...
describe('httpClient', () => {
  ...
  test('should successfully fetch JSON data', async () => {
    ...
    const result = await httpClient<{ data: string }>('https://api.example.com/data');
  ...
  test('should throw an error for non-OK responses', async () => {
    ...
    await expect(httpClient('https://api.example.com/notfound')).rejects.toThrow('HTTP error! status: 404, statusText: Not Found');
  });
  ...
  test('should throw an error for unexpected content types', async () => {
    ...
    await expect(httpClient('https://api.example.com/text')).rejects.toThrow('Unexpected content type: text/plain');
  });
});
...
describe('createHttpClient', () => {
  ...
  test('should create a client with a base URL', async () => {
    ...
    expect(result).toEqual({ data: 'test' });
    expect(mockedFetch).toHaveBeenCalledWith('https://api.example.com/data', { method: 'GET' });
  });
});
